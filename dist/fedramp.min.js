...
$stateProvider.state('fedramp',{abstract:true,templateUrl:'src/fedramp/fedramp.html',resolve:{fedrampData:FedRampData}}).state('fedramp.home',{url:'',templateUrl:'src/fedramp/home/home.html',controller:'HomeController as homeController'}).state('fedramp.home.provider',{url:'/provider',templateUrl:'src/fedramp/home/provider.html',controller:'ProviderController as controller'}).state('fedramp.home.provider.information',{url:'/provider/:name',templateUrl:'src/fedramp/home/provider-information.html',controller:'ProviderInformationController as controller'}).state('fedramp.home.provider.information.comparison',{url:'/provider/:first/versus/:second',templateUrl:'src/fedramp/home/provider-comparison.html',controller:'ProviderComparisonController as controller'}).state('fedramp.home.products',{url:'/products',templateUrl:'src/fedramp/home/product.html',controller:'ProductController as controller',resolve:{products:ProductsResolve}})//.state('fedramp.product', {
//url: '/product/:name',
...
//}
//})
.state('fedramp.home.product',{url:'/product'}).state('fedramp.home.product.information',{url:'/product/:name',templateUrl:'src/fedramp/home/product-information.html',controller:'ProductInformationController as controller'}).state('fedramp.home.product.information.comparison',{url:'/product/:first/versus/:second',templateUrl:'src/fedramp/home/product-comparison.html',controller:'ProductComparisonController as controller'}).state('fedramp.home.agency',{url:'/agency',templateUrl:'src/fedramp/home/agency.html',controller:'AgencyController as controller'}).state('fedramp.home.agency.information',{url:'/agency/:name',templateUrl:'src/fedramp/home/agency-information.html',controller:'AgencyInformationController as controller'}).state('fedramp.home.agency.information.comparison',{url:'/agency/:first/versus/:second',templateUrl:'src/fedramp/home/agency-comparison.html',controller:'AgencyComparisonController as controller'}).state('fedramp.home.assessor',{url:'/assessor',templateUrl:'src/fedramp/home/assessor.html',controller:'AssessorController as controller'}).state('fedramp.home.assessor.information',{url:'/assessor/:name',templateUrl:'src/fedramp/home/assessor-information.html',controller:'AssessorInformationController as controller'}).state('fedramp.home.assessor.information.comparison',{url:'/assessor/:first/versus/:second',templateUrl:'src/fedramp/home/assessor-comparison.html',controller:'AssessorComparisonController as controller'});/**
         * Retrieves producsts for Product route
         */ProductsResolve.$inject=['fedrampData'];function ProductsResolve(fedrampData){return fedrampData.products();}/**
         * Retrieves the providers for a particular day
         */FedRampData.$inject=['DataService'];function FedRampData(DataService){return DataService.pull().then(function(storage){return storage;});}}})();(function(){'use strict';angular.module('fedramp.services').component('tagField',{templateUrl:'src/fedramp.components/tag-field.html',controller:TagField,controllerAs:'controller',bindings:{items:'<',selectedItems:'='}});TagField.$inject=['$log'];function TagField($log){var self=this;var selectedCache={};self.$onInit=onInit;self.filterSelected=filterSelected;self.onItemSelect=onItemSelect;self.deselect=deselect;// self.$onChanges = function (changes) {};
// self.$onDestroy = function () {};
// self.$postLink = function () {};
//
function onItemSelect(){$log.info(self.selectedItem);if(!self.selectedItem){return null;}if(!selectedCache[self.selectedItem]){self.selectedItems.push(self.selectedItem);selectedCache[self.selectedItem]=true;}}function deselect(item){var pos=self.selectedItems.indexOf(item);selectedCache[item]=false;self.selectedItems.splice(pos,1);}function filterSelected(value,index,array){if(selectedCache[value]){return null;}return value;}function onInit(){self.selectedItems=[];}}})();(function(){'use strict';angular.module('fedramp.services').component('tile',{templateUrl:'src/fedramp.components/tile.html',controller:Tile,controllerAs:'controller',bindings:{expand:'<',model:'<'}});Tile.$inject=['$log'];function Tile($log){// var self = this;
...
var self=this;self.type='product';self.name='';self.reuses=0;self.provider='';self.agencies=[];self.deploymentModel='';self.designation='';self.serviceModels=[];self.impactLevel='';}return Product;}})();(function(){'use strict';angular.module('fedramp.models').factory('Provider',ProviderFactory);ProviderFactory.$inject=[];function ProviderFactory(){/**
...
var self=this;/**
     ...
     */function DatasourceService($http){var self=this;/**
         ...
         */function StorageData(options){StorageManager.call(this);var self=this;self.storageContainer='data';/**
             ...
             */self.providers=function(){var names=[];var items=[];var data=self.all();for(var i=0;i<data.length;i++){var d=data[i];if(!d.name||names.includes(d.name)){continue;}names.push(d.name);var item=new Provider();item.name=d.name;items.push(item);}items.forEach(function(item){item.products=self.products().filter(function(x){return x.name===item.name;});item.products.forEach(function(prod){item.reuses+=prod.reuses;item.products.forEach(function(prod){prod.serviceModels.forEach(function(model){model=model.trim();if(!item.serviceModels.includes(model)){item.serviceModels.push(model);}});});item.products.forEach(function(prod){prod.deplomentModels.forEach(function(model){model=model.trim();if(!item.deploymentModels.includes(model)){item.deploymentModels.push(model);}});});item.products.forEach(function(prod){if(!item.designations.includes(prod.deploymentModel)){item.designations.push(prod.deploymentModel);}});});});return items;};/**
             ...
             */self.products=function(){var names=[];var items=[];var data=self.all();for(var i=0;i<data.length;i++){var d=data[i];if(!d.pkg||names.includes(d.pkg)){continue;}names.push(d.pkg);var item=new Product();item.name=d.pkg;item.provider=d.name;item.serviceModels=d.serviceModel.map(function(x){return x.trim();});item.deploymentModel=d.deploymentModel;item.designation=d.designation;item.impactLevel=d.impactLevel;items.push(item);}items.forEach(function(item){data.forEach(function(d){if(d.pkg===item.name){if(d.sponsoringAgency&&!item.agencies.includes(d.sponsoringAgency)){item.agencies.push(d.sponsoringAgency);}if(d.sponsoringSubagency&&!item.agencies.includes(d.sponsoringSubagency)){item.agencies.push(d.sponsoringSubagency);}d.atoLetters.forEach(function(a){if(!item.agencies.includes(a.authorizingAgency)){item.agencies.push(d.authorizingAgency);}if(!item.agencies.includes(a.authorizingSubagency)){item.agencies.push(d.authorizingSubagency);}});}});});return items;};/**
...
summedReuses+=leveragedATOs.map(function(otherCSP){return otherCSP.atoLetters.length;}).reduce(function(prev,curr){return prev+curr;});}return directlyLeveraged+summedReuses;};/**
         ...
         */function sortData(){if(!self.sortBy||self.sortBy.length===0){return;}self.filteredData=self.filteredData.sort(function(a,b){if(a[self.sortBy]===b[self.sortBy]){return 0;}if(self.sortAscending){return a[self.sortBy]<b[self.sortBy]?-1:1;}return a[self.sortBy]>b[self.sortBy]?-1:1;});}}})();(function(){"use strict";angular.module('fedramp').controller('ProductController',ProductController);ProductController.$inject=['$log','products','$stateParams'];function ProductController($log,products,$stateParams){var self=this;self.filterOptionGroups=[];// Agencies
...
self.selectedAgencies=[];self.selectedServiceModels=[];// Assign products resolve to controller
...
var providerCache={};self.products.forEach(function(value,index,arr){if(!providerCache[value.provider]){value.type='Product Name';self.filterOptionGroups.push({name:value.provider,type:'Provider'});providerCache[value.provider]=true;}});// Populate Agency list
...
self.serviceModels=['IaaS','PaaS','SaaS'];self.serviceModels.forEach(function(value,index,arr){self.filterOptionGroups.push({name:value,type:'Service Model'});});}/**
         ...
         */function filterAgency(value,index,arr){if(self.selectedAgencies&&self.selectedAgencies.length>0&&value.agencies){var found=false;for(var x=0;x<self.selectedAgencies.length;x++){var curValue=self.selectedAgencies[x];if(value.agencies.includes(curValue)){console.log(curValue);found=true;}}if(!found){return null;}}return value;}function filterSelected(){if(!filterCache[self.selectedFilter.name]){switch(self.selectedFilter.type){case'Provider':self.selectedNames.push(self.selectedFilter.name);filterCache[self.selectedFilter.name]=true;break;case'Agency':console.log('Agency case: ',self.selectedFilter);self.selectedAgencies.push(self.selectedFilter.name);filterCache[self.selectedFilter.name]=true;break;case'Service Model':self.selectedServiceModels.push(self.selectedFilter.name);filterCache[self.selectedFilter.name]=true;break;}}console.log(self.selectedAgencies);}function removeProviderFilter(filter){console.log(filter);var pos=self.selectedNames.indexOf(filter);console.log(pos);self.selectedNames.splice(pos,1);filterCache[filter]=false;}function removeAgencyFilter(filter){var pos=self.selectedAgencies.indexOf(filter);console.log(pos);self.selectedAgencies.splice(pos,1);filterCache[filter]=false;}function removeServiceModelFilter(filter){var pos=self.selectedServiceModels.indexOf(filter);console.log(pos);self.selectedServiceModels.splice(pos,1);filterCache[filter]=false;}// Initialize everything
onInit();}})();(function(){})();