...
placeholder:'@',id:'@',filterFunc:'<'}});GridSearch.$inject=['Searcher'];/**
     ...
     */function GridSort($log,$parse,$element,$location){var self=this;self.activated=null;self.asc=true;self.$onInit=$onInit;self.sort=sort;self.highlight=highlight;self.toggleSort=toggleSort;self.clear=clear;function $onInit(){if(!self.name){throw'Name must be specified for sort filter';}if(self.property){self.sortFunc=sortFunc;}self.propertyParser=$parse(self.property);restoreState();self.gridController.addSort(self);}/**
...
self.gridController.updateSort();saveState();}/**
         ...
         */function value(obj){var value=self.propertyParser(obj);if(angular.isString(value)){return value.toLowerCase().trim();}return value;}/**
