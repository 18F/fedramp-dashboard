...
id:'@'}});GridFilter.$inject=['$location','$parse','$element'];/**
     * @constructor
     * @memberof Components
     * @example 
...
if(self.options.length===0){self.loadOptions(self.gridController.rawItems);}if(self.gridController.state){var selected=loadSavedOptions();if(selected){self.selectedOptionValues=selected;applyFilter();}}}function $postLink(){if(self.expanded){$element.addClass('grid-filter-expanded');}}/**
         ...
         * adds them
         */function loadSavedOptions(){var params=self.gridController.state;if(!(self.id in params)){return null;}var values=params[self.id];var selected=[];values.split(',').forEach(function(val){selected.push({value:val,selected:true});self.options.forEach(function(option){if(option.value===val){option.selected=true;}});});return selected;}/**
         * Toggles the selection of an option and then executes filter.
         * @public
         * @memberof Components.GridFilter
         */function selectOption(option){option.selected=!option.selected;var pos=self.selectedOptionValues.findIndex(function(x){return x.value===option.value;});if(pos==-1){self.selectedOptionValues.push(option);}else{self.selectedOptionValues.splice(pos,1);}if(self.gridController.state){saveState();}applyFilter();}/**
         ...
         */function saveState(){if(self.selectedOptionValues&&self.selectedOptionValues.length>0){var options=self.selectedOptionValues.map(function(option){return option.value;}).join(',');self.gridController.state[self.id]=options;}else{delete self.gridController.state[self.id];}}/**
