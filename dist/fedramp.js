...
var self=this;self.type='product';self.name='';self.pkgId='';self.reuses=0;self.provider='';self.agencies=[];self.deploymentModel='';self.designation='';self.serviceModels=[];self.impactLevel='';self.logo='';self.authorizationDate='';self.expectedCompliance='';}return Product;}})();(function(){'use strict';angular.module('fedramp.models').factory('Provider',ProviderFactory);ProviderFactory.$inject=[];function ProviderFactory(){/**
         * The cloud service provider.
         * @constructor
         * @memberof Models
         */function Provider(options){// Scope `this` to self
var self=this;self.type='provider';self.name='';self.reuses=0;self.products=[];self.deploymentModels=[];self.serviceModels=[];self.designations=[];self.logo='';}return Provider;}})();(function(){'use strict';angular.module('fedramp.services').factory('Settings',SettingsFactory);function SettingsFactory(){return Settings;}/**
...
var self=this;/**
     ...
     * @constructor
     * @memberof Services
     */function HelperService($log){var self=this;/**
         ...
         * @constructor
         * @memberof Services
         */function Searcher(data,propExpression){var self=this;var targetKey=null;var targetProps=null;var isPrimitive=true;var useIndex=false;// public funcs
self.search=search;function init(){parseKeys();}function parseKeys(){var m=propExpression.match(ARRAY_FILTER_REGEX);if(m){targetKey=m[1].split('.').splice(1).join('.');targetProps=m[2].split('.');isPrimitive=false;useIndex=targetKey==='';}else{targetKey=propExpression;}}/**
             ...
             */function search(q){if(angular.isArray(data)){return data.filter(function(obj){var found=find(obj,targetProps,q);if(found){return obj;}return null;});}var found=find(data,targetProps,q);if(found){return data;}return null;}/**
             ...
             */function find(obj,props,q){if(!props){return match($parse(targetKey)(obj),q);}props=angular.copy(props);if(props.length===0){if(isPrimitive||useIndex){return match(obj,q);}return match($parse(targetKey)(obj),q);}var curProp=props.shift();var value=$parse(curProp)(obj);if(angular.isArray(value)){for(var x=0;x<value.length;x++){var found=find(value[x],props,q);if(found){return found;}}}return false;}/**
             * The default comparator when determining if a search is a match
             */function match(value,searchTerm){var pos=value.toString().toLowerCase().indexOf(searchTerm.toString().toLowerCase());return pos!==-1;}init();}}})();(function(){'use strict';angular.module('fedramp.services').factory('StorageData',StorageDataFactory);StorageDataFactory.$inject=['StorageManager','Data','Agency','Assessor','Product','Provider','helperService'];function StorageDataFactory(StorageManager,Data,Agency,Assessor,Product,Provider,helperService){/**
         ...
         */function StorageData(options){StorageManager.call(this);var self=this;self.storageContainer='data';/**
             ...
             */self.providers=function(){var names=[];var items=[];var data=self.all();for(var i=0;i<data.length;i++){var d=data[i];if(!include(d.name,names)){continue;}names.push(d.name.trim());var item=new Provider();item.name=d.name.trim();item.logo=d.cspUrl;items.push(item);}items.forEach(function(item){item.products=self.products().filter(function(x){return x.provider===item.name;});item.products.forEach(function(prod){prod.name=prod.name.trim();item.reuses+=prod.reuses;item.products.forEach(function(prod){if(prod.serviceModels){prod.serviceModels.forEach(function(model){if(include(model,item.serviceModels)){item.serviceModels.push(model.trim());}});}});item.products.forEach(function(prod){if(prod.deploymentModel){if(include(prod.deploymentModel,item.deploymentModels)){item.deploymentModels.push(prod.deploymentModel.trim());}}});item.products.forEach(function(prod){if(include(prod.designation,item.designations)){item.designations.push(prod.designation.trim());}});});});return items;};/**
...
self.$onInit=$onInit;self.applyFilter=applyFilter;self.selectOption=selectOption;self.clear=clear;self.doFilter=doFilter;self.loadOptions=loadOptions;self.$postLink=$postLink;//Array parsing expressions
...
return objectFilterFunc(obj,index,arr);}/**
         ...
         * @public
         * @memberof Components.GridFilter
         */function clear(){self.selectedOptionValues=[];self.options.forEach(function(x){return x.selected=false;});applyFilter();}}})();(function(){'use strict';angular.module('fedramp.components').component('gridSearch',{templateUrl:'src/fedramp.components/grid-search.html',controller:GridSearch,controllerAs:'controller',require:{gridController:'^grid'},bindings:{// Property expression. Tells search which property to search on. 
property:'@',placeholder:'@'}});GridSearch.$inject=['Searcher'];/**
     ...
     * @constructor
     * @memberof Components
     */function GridSearch(Searcher){var self=this;self.filtered=[];self.$onInit=$onInit;self.search=search;self.doFilter=doFilter;function $onInit(){self.gridController.addFilter(self);}/**
         * Filter applied to dataset when user searches
         */function doFilter(obj,index,arr){if(self.searchTerm){var found=new Searcher(obj,self.property).search(self.searchTerm);return found;}else{return obj;}}/**
         ...
         */function search(){var filtered=self.gridController.rawItems.filter(self.doFilter);self.filtered=filtered;self.gridController.doFilter();}}})();(function(){'use strict';angular.module('fedramp.components').component('gridSort',{templateUrl:'src/fedramp.components/grid-sort.html',require:{gridController:'^grid'},controller:GridSort,controllerAs:'controller',bindings:{property:'@',caseSensitive:'@',header:'@',default:'@'}});GridSort.$inject=['$log','$parse'];/**
     * @constructor
     * @memberof Components
...
self.gridController.updateSort();}/**
         ...
         */function value(obj){var value=self.propertyParser(obj);if(angular.isString(value)){return value.toLowerCase().trim();}return value;}function highlight(asc){if(!self.activated){return false;}if(self!==self.gridController.defaultSort){return false;}if(self.asc===asc){return true;}return false;}}})();(function(){'use strict';angular.module('fedramp.components').component('grid',{controller:Grid,controllerAs:'gridController',bindings:{// Contains all the filtered items
...
self.defaultSort=null;function $onInit(){if(!self.items){self.items=self.rawItems;}}/**
...
combinedFilterResults=combinedFilterResults.filter(filter.doFilter);});// Apply default sort if one exists
if(self.defaultSort){combinedFilterResults.sort(self.defaultSort.sortFunc);}self.updateSort();self.items=combinedFilterResults;}/**
         ...
         * activated.
         */function updateSort(){sorts.forEach(function(sort){if(!angular.equals(sort,self.defaultSort)){sort.activated=false;}});}}})();(function(){'use strict';angular.module('fedramp.components').component('product',{templateUrl:'src/fedramp.components/product.html',controller:Product,controllerAs:'controller',bindings:{model:'<',onClose:'<'}});Product.$inject=['$log','$state'];/**
     * @constructor
     * @memberof Components
