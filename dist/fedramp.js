...
var self=this;/**
     ...
     */function DatasourceService($http){var self=this;/**
         ...
         */function StorageData(options){StorageManager.call(this);var self=this;self.storageContainer='data';/**
             ...
             */self.providers=function(){var names=[];var items=[];var data=self.all();for(var i=0;i<data.length;i++){var d=data[i];if(!include(d.name,names)){continue;}names.push(d.name.trim());var item=new Provider();item.name=d.name.trim();items.push(item);}items.forEach(function(item){item.products=self.products().filter(function(x){return x.name===item.name;});if(!item.products){item.products=[];}item.products.forEach(function(prod){item.reuses+=prod.reuses;item.products.forEach(function(prod){if(prod.serviceModels){prod.serviceModels.forEach(function(model){if(include(model,item.serviceModels)){item.serviceModels.push(model.trim());}});}});item.products.forEach(function(prod){if(prod.deploymentModels){prod.deplomentModels.forEach(function(model){if(include(model,item.deploymentModels)){item.deploymentModels.push(model.trim());}});}});item.products.forEach(function(prod){if(include(prod.designation,item.designations)){item.designations.push(prod.designation.trim());}});});});return items;};/**
