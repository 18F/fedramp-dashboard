...
var match=fromState.name.match(STATE_REGEX);if(match){$location.search(params);}});}})();(function(){'use strict';angular.module('fedramp').config(routeConfig);// Add items to inject for safe minification
...
$stateProvider.state('fedramp',{abstract:true,templateUrl:'src/templates/base.html',resolve:{fedrampData:['DataService',function(DataService){return DataService.pull().then(function(storage){return storage;});}]}}).state('fedramp.sitemap',{url:'/sitemap.xml',templateUrl:'src/templates/sitemap/sitemap.html',controller:'SitemapController as controller'}).state('fedramp.app',{abstract:true,templateUrl:'src/templates/fedramp.html'}).state('fedramp.app.search',{url:'/search/:query',templateUrl:'src/templates/search/search.html',controller:'SearchController as controller'}).state('fedramp.app.home',{templateUrl:'src/templates/home/home.html',controller:'HomeController as homeController'})// Provider States
...
var self=this;/**
         ...
         */function today(){var d=new Date();var dd=d.getDate();var mm=d.getMonth()+1;var yyyy=d.getFullYear();if(dd<10){dd='0'+dd;}if(mm<10){mm='0'+mm;}return mm+'/'+dd+'/'+yyyy;}return self.init(options);}})();(function(){'use strict';angular.module('fedramp.components').component('agency',{templateUrl:'src/templates/components/agency.html',controller:Agency,controllerAs:'controller',bindings:{model:'<',onClose:'<'}});Agency.$inject=['$log','$state','helperService'];/**
     * @constructor
     * @memberof Components
...
$location.search(self.state);self.onUpdate({items:combinedFilterResults,state:self.state});}/**
         ...
         */function stateUpdate(state){state=angular.extend(self.state,state);$location.search(state);self.onUpdate({items:self.items,state:state});}}})();(function(){'use strict';angular.module('fedramp.components').component('productStatus',{templateUrl:'src/templates/components/product-status.html',controller:ProductStatus,controllerAs:'controller',bindings:{status:'<'}});ProductStatus.$inject=[];/**
