...
opened:'<'}});GridFilter.$inject=['$location'];function GridFilter($location){var self=this;self.gridFilterOptionsTemplatePath="src/fedramp.components/grid-filter-options.html";// Options available to filter based on property
...
self.$onInit=$onInit;self.$onChanges=$onChanges;self.filterFunc=filterFunc;self.applyFilter=applyFilter;self.selectOption=selectOption;self.clear=clear;self.reloadOptions=reloadOptions;self.loadInitialValues=loadInitialValues;function $onInit(){// We give the parent controller a reference to this filter
...
if(self.initialValues&&self.initialValues.length>0){loadInitialValues(self.initialValues);applyFilter();}if(self.gridController.savedState){var initialValues=loadSavedOptions();if(!initialValues||initialValues.length===0){return;}console.log('Doing stuff',initialValues.split(','));loadInitialValues(initialValues.split(','));applyFilter();}console.log('$gridFilter $onInit');}function $onChanges(changes){}function loadSavedOptions(){var params=$location.search();if(!(self.property in params)){return null;}var values=params[self.property];return values;}/**
...
if(angular.isString(property)||angular.isNumber(property)){return objectFilterFunc(obj,index,arr);}throw'Invalid object. Cannot filter';}/**
         ...
         */function arrayFilterFunc(obj,index,arr){var values=obj[self.property];var found=false;values.forEach(function(value){var foundObject=self.selectedOptionValues.find(function(option){console.log('arrayFilterFunc');return option.value.trim()===value.trim();});if(foundObject){found=true;}});if(found){return obj;}return null;}/**
...
if(angular.isString(property)||angular.isNumber(property)){set.add(property);}});self.options=toOptionArray(set);}/**
         ...
         */function clear(){self.selectedOptionValues=[];self.options.forEach(function(x){return x.selected=false;});applyFilter();}}})();(function(){'use strict';angular.module('fedramp.components').component('gridSort',{transclude:{sort:'?sort'},templateUrl:'src/fedramp.components/grid-sort.html',require:{gridController:'^grid'},controller:GridSort,controllerAs:'controller',bindings:{property:'@',sortFunc:'<',caseSensitive:'@',header:'@'}});GridSort.$inject=['$transclude'];function GridSort($transclude){var self=this;var dir=-1;self.asc=true;self.$onInit=$onInit;self.sort=sort;function $onInit(){if(self.property){self.sortFunc=sortFunc;}if(!self.sortFunc){throw'Must specify sort property or sort function';}}self.filled=function(){return $transclude.isSlotFilled('sort');};function sort(doAscending){self.asc=doAscending;self.gridController.items.sort(self.sortFunc);}function sortFunc(_a,_b){var a=value(_a);var b=value(_b);if(angular.isNumber(a)){return numberSortFunc(a,b);}if(a<b){return self.asc?-1:1;}if(a>b){return self.asc?1:-1;}return 0;}function numberSortFunc(a,b){if(self.asc){return a-b;}return b-a;}function value(obj){var value=obj[self.property];if(angular.isString(value)){return value.toLowerCase().trim();}return value;}}})();(function(){'use strict';angular.module('fedramp.components').component('grid',{controller:Grid,controllerAs:'gridController',bindings:{// Contains all the filtered items
...
if(self.savedState){if(filter.selectedOptionValues&&filter.selectedOptionValues.length>0){$location.search(filter.property,filter.selectedOptionValues.map(function(x){return x.value;}).join(','));}}});self.items=combinedFilterResults;}function clearFilters(){filters.forEach(function(filter){filter.clear();});}/**
...
navigator.msSaveBlob(downloadBlob,self.filename());}};/**
     ...
     * @constructor
     * @memberof Controllers
     */function ProvidersController($log,providers){var self=this;self.providers=providers;$log.debug(providers);}})();