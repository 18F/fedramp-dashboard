...
}})();(function(){'use strict';angular.module('fedramp.components').component('gridFilterClear',{transclude:true,template:'<ng-transclude ng-click="controller.clear()"></ng-transclude>',controller:GridFilterClear,controllerAs:'controller',require:{gridController:'^grid'},bindings:{}});GridFilterClear.$inject=[];/**
     * @constructor
     * @memberof Components
...
expanded:'<'}});GridFilter.$inject=['$filter'];function GridFilter($filter){var self=this;self.gridFilterOptionsTemplatePath="src/fedramp.components/grid-filter-options.html";// Options available to filter based on property
...
self.filtered=[];// Exposed public functions
self.$onInit=$onInit;self.$onChanges=$onChanges;self.filterFunc=filterFunc;self.selectOption=selectOption;self.clear=clear;function $onInit(){// We give the parent controller a reference to this filter
...
if(self.initialValues&&self.initialValues.length>0){loadInitialValues(self.initialValues);applyFilter();}}function $onChanges(changes){}/**
         * Toggles the selection of an option and then executes filter.
         */function selectOption(option){option.selected=!option.selected;var pos=self.selectedOptionValues.indexOf(option);if(pos==-1){self.selectedOptionValues.push(option);}else{self.selectedOptionValues.splice(pos,1);}applyFilter();}/**
         ...
         */function applyFilter(){self.filtered=$filter('filter')(self.gridController.rawItems,self.filterFunc);self.gridController.doFilter();}/**
...
if(angular.isString(property)){return objectFilterFunc(obj,index,arr);}throw'Invalid object. Cannot filter';}/**
         ...
         */function arrayFilterFunc(obj,index,arr){var values=obj[self.property];var found=false;values.forEach(function(value){var foundObject=self.selectedOptionValues.find(function(option){return option.value===value;});if(foundObject){found=true;}});if(found){return obj;}return null;}/**
         ...
         */function loadDefaultOptions(){var set=new Set();self.gridController.rawItems.forEach(function(obj){var property=obj[self.property];// Handle when object is array
...
if(angular.isString(property)||angular.isNumber(property)){set.add(property);}});//self.options = Array.from(set);
self.options=toOptionArray(set);}/**
         ...
         */function loadInitialValues(initialValues){self.selectedOptionValues=initialValues;self.selectedOptionValues.forEach(function(value){self.options.forEach(function(option){if(option.value===value){option.selected=true;}});});}/**
         * Clears filter and resets dataset
         */function clear(){self.selectedOptionValues=[];self.options.forEach(function(x){return x.selected=false;});applyFilter();}}})();(function(){'use strict';angular.module('fedramp.components').component('gridSort',{transclude:true,template:'<ng-transclude ng-click="controller.sort()"></ng-transclude>',require:{gridController:'^grid'},controller:GridSort,controllerAs:'controller',bindings:{property:'@',sortFunc:'<'}});function GridSort(){var self=this;var dir=-1;self.asc=true;self.$onInit=$onInit;self.sort=sort;function $onInit(){if(self.property){self.sortFunc=sortFunc;return;}if(!self.sortFunc){throw'Must specify sort property or sort function';}console.log(self.gridController.items);}function sort(){self.gridController.items.sort(self.sortFunc);self.asc=!self.asc;}function sortFunc(_a,_b){var a=value(_a);var b=value(_b);if(a<b){return self.asc?-1:1;}if(a>b){return self.asc?1:-1;}return 0;}function value(obj){return obj[self.property];}}})();(function(){'use strict';angular.module('fedramp.components').component('grid',{controller:Grid,controllerAs:'gridController',bindings:{items:'=',rawItems:'<'}});Grid.$inject=['$log','$filter'];/**
     * @constructor
     * @memberof Components
     */function Grid($log,$filter){var self=this;// Maintains a list of filters
var filters=[];self.addFilter=addFilter;self.doFilter=doFilter;self.clearFilters=clearFilters;/**
...
if(filter.filtered.length===0){return;}combinedFilterResults=$filter('filter')(combinedFilterResults,filter.filterFunc);});self.items=combinedFilterResults;}function clearFilters(){filters.forEach(function(filter){filter.clear();});}/**
     ...
     */function Provider($log,$state){var self=this;/**
         ...
         */self.close=function(){if(self.onClose){self.onClose();return;}$state.go('fedramp.app.home',{},{reload:true});};}})();(function(){'use strict';angular.module('fedramp.components').component('tile',{templateUrl:'src/fedramp.components/tile.html',controller:Tile,controllerAs:'controller',bindings:{expand:'<',model:'<'}});Tile.$inject=['$log','$state','$stateParams','helperService'];/**
     * @constructor
     * @memberof Components
...
navigator.msSaveBlob(downloadBlob,self.filename());}};/**
     ...
     */function ProductInformationController($log,$state,$stateParams,fedrampData,helperService){var self=this;/**
         ...
         * @member {object}
         * @memberof Controllers.ProductInformationController
         */self.close=function(){$state.go('fedramp.app.home',{},{reload:true});};}})();(function(){"use strict";angular.module('fedramp').controller('ProductsController',ProductController);ProductController.$inject=['$log','products','$stateParams','$filter','$location'];function ProductController($log,products,$stateParams,$filter,$location){var self=this;self.testItems=[{name:'Ace',type:'Provider'},{name:'Winston',type:'Agency'},{name:'John',type:'Agency'},{name:'Bryans',type:'Product'}];self.filterOptionGroups=[];// Agencies
self.agencies=[];self.providers=[];// Product Names
...
self.products=products;console.log(self.savedServiceModels);// Funcs
self.filterName=filterName;self.filterServiceModel=filterServiceModel;self.filterAgency=filterAgency;self.filterSelected=filterSelected;self.removeProviderFilter=removeProviderFilter;self.removeAgencyFilter=removeAgencyFilter;self.removeServiceModelFilter=removeServiceModelFilter;self.addServiceModelFilter=addServiceModelFilter;self.sortDeploymentModel=function(a,b){if(a.deploymentModel<b.deploymentModel){return-1;}if(a.deploymentModel>b.deploymentModel){return 1;}return 0;};// Used to prevent duplicate options
var filterCache={};function onInit(){// Populate Product names
var providerCache={};self.products.forEach(function(value,index,arr){if(!providerCache[value.provider]){self.providers.push(value.provider);providerCache[value.provider]=true;}});// Populate Agency list
var agencyMap={};self.products.forEach(function(element,index,array){if(element&&element.agencies){element.agencies.forEach(function(agency){if(agency&&!agencyMap[agency]){self.agencies.push(agency);//self.filterOptionGroups.push({
//name: agency,
//type: 'Agency'
//});
agencyMap[agency]=true;}});}});// Populate service models
self.serviceModels=['IaaS','PaaS','SaaS'];self.filterOptionGroups.sort(function(a,b){if(a.name===b.name){return 0;}if(a.name<b.name){return-1;}return 1;});console.log(self.filteredData);//applyFilters();
self.savedServiceModels=$location.search().serviceModels?$location.search().serviceModels.split(','):[];self.savedAgencies=$location.search().agencies?$location.search().agencies.split(','):[];self.filteredData=angular.copy(self.products);}/**
...
onInit();}})();(function(){'use strict';angular.module('fedramp').controller('ProviderComparisonController',ProviderComparisonController);ProviderComparisonController.$inject=['$log','$state','$stateParams','fedrampData','helperService'];/**
     ...
     * @constructor
     * @memberof Controllers
     */function ProvidersController($log,providers){var self=this;self.providers=providers;}})();(function(){'use strict';angular.module('fedramp').controller('SitemapController',SitemapController);SitemapController.$inject=['$log','fedrampData','helperService'];/**
     * @constructor
     * @memberof Controllers
