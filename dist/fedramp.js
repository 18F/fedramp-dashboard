...
if(self.options.length===0){self.loadOptions(self.gridController.rawItems);}if(self.gridController.savedState){var selected=loadSavedOptions();if(selected){self.selectedOptionValues=selected;applyFilter();}}}function $postLink(){if(self.expanded){$element.addClass('grid-filter-expanded');}}/**
...
property:'@',placeholder:'@',id:'@',filterFunc:'<'}});GridSearch.$inject=['Searcher'];/**
     ...
     */function GridSearch(Searcher){var self=this;var searcher=new Searcher();self.filtered=[];self.$onInit=$onInit;self.search=search;function $onInit(){if(!self.id){throw'Id is required for grid search filter';}if(!self.property&&!self.filterFunc){throw'If property is not specified, filterFunc must be provided';}self.filterFunc=self.filterFunc?wrapFilterFunc(self.filterFunc):filterFunc;self.gridController.addFilter(self);}/**
         ...
         */function search(){var filtered=self.gridController.rawItems.filter(self.filterFunc);self.filtered=filtered;self.gridController.doFilter();}function wrapFilterFunc(func){return function(obj,index,arr){return func(obj,index,arr,self.searchTerm);};}}})();(function(){'use strict';angular.module('fedramp.components').component('gridSort',{templateUrl:'src/fedramp.components/grid-sort.html',require:{gridController:'^grid'},controller:GridSort,controllerAs:'controller',bindings:{property:'@',caseSensitive:'@',header:'@',default:'@'}});GridSort.$inject=['$log','$parse','$element'];/**
     * @constructor
     * @memberof Components
     * @example <grid-sort property="provider" header="Provider"></grid-sort>
     */function GridSort($log,$parse,$element){var self=this;self.activated=null;self.asc=true;self.$onInit=$onInit;self.sort=sort;self.highlight=highlight;self.toggleSort=toggleSort;self.clear=clear;function $onInit(){if(self.property){self.sortFunc=sortFunc;}self.propertyParser=$parse(self.property);if(self.default){// Parse if default should be in asc/desc
self.asc=self.default.split('-').reduce(function(p,c){return false;});self.sort(self.asc);}self.gridController.addSort(self);}/**
...
savedState:'<',onUpdate:'&'}});Grid.$inject=['$log','$location'];/**
     * @constructor
     * @memberof Components
...
var filters=[];var sorts=[];self.$onInit=$onInit;self.addFilter=addFilter;self.addSort=addSort;self.doFilter=doFilter;self.clearFilters=clearFilters;self.updateSort=updateSort;// Default sort to utilize for results
...
if(self.defaultSort){combinedFilterResults.sort(self.defaultSort.sortFunc);}self.updateSort();self.items=combinedFilterResults;// Pass updated dataset
self.onUpdate({items:combinedFilterResults});}/**
         ...
         * activated.
         */function updateSort(){sorts.forEach(function(sort){if(!angular.equals(sort,self.defaultSort)){sort.clear();}});//self.onUpdate({items: self.items});
...
var self=this;/**
     ...
     */function DatasourceService($http){var self=this;/**
         ...
         * @returns
         *  The response as a promise
         */self.pull=function(url){return $http.get(url).then(function(response){return response.data;});};}})();(function(){'use strict';angular.module('fedramp.services').service('helperService',HelperService);HelperService.$inject=['$log'];/**
     * @constructor
     * @memberof Services
     */function HelperService($log,$location,$anchorScroll){var self=this;/**
...
setTimeout(function(){var el=document.getElementById(anchor);if(!el){return;}var y=el.offsetTop;var node=el;while(node.offsetParent&&node.offsetParent!=document.body){node=node.offsetParent;y+=node.offsetTop;}scrollTo(0,y);},100);}};}})();(function(){"use strict";angular.module('fedramp.services').factory('Searcher',SearcherFactory);SearcherFactory.$inject=['$log','$parse'];function SearcherFactory($log,$parse){var ARRAY_FILTER_REGEX=/^(.*)\sin\s(.+)$/;return Searcher;/**
...
if(response&&response.data){if(response.data.results){self.results=response.data.results;}}},function(response){self.error=true;});})();}})();(function(){'use strict';angular.module('fedramp').controller('SitemapController',SitemapController);SitemapController.$inject=['$log','fedrampData','helperService'];/**
     ...
     */function AgencyComparisonController($log,$state,$stateParams,fedrampData,helperService){var self=this;/**
         ...
         * @member {object}
         * @memberof Controllers.AgencyComparisonController
         */self.closeSecond=function(){$state.go('fedramp.app.'+self.first.type+'.information',{name:$stateParams.first},{reload:true});};helperService.scrollTo('scrollToContent');}})();(function(){})();(function(){'use strict';angular.module('fedramp').controller('AgencyInformationController',AgencyInformationController);AgencyInformationController.$inject=['$log','$state','$stateParams','fedrampData','helperService'];/**
     * @constructor
     * @memberof Controllers
     */function AgencyInformationController($log,$state,$stateParams,fedrampData,helperService){var self=this;/**
         ...
         * @member {object}
         * @memberof Controllers.AgencyInformationController
         */self.close=function(){$state.go('fedramp.app.home.agencies',{},{reload:true});};helperService.scrollTo('scrollToContent');}})();(function(){'use strict';angular.module('fedramp').controller('AssessorComparisonController',AssessorComparisonController);AssessorComparisonController.$inject=['$log','$state','$stateParams','fedrampData','helperService'];/**
     * @constructor
     * @memberof Controllers
     */function AssessorComparisonController($log,$state,$stateParams,fedrampData,helperService){var self=this;/**
         ...
         * @member {object}
         * @memberof Controllers.AssessorComparisonController
         */self.closeSecond=function(){$state.go('fedramp.app.'+self.first.type+'.information',{name:$stateParams.first},{reload:true});};helperService.scrollTo('scrollToContent');}})();(function(){})();(function(){'use strict';angular.module('fedramp').controller('AssessorInformationController',AssessorInformationController);AssessorInformationController.$inject=['$log','$state','$stateParams','fedrampData','helperService'];/**
     * @constructor
     * @memberof Controllers
     */function AssessorInformationController($log,$state,$stateParams,fedrampData,helperService){var self=this;/**
         ...
         * @member {object}
         * @memberof Controllers.AssessorInformationController
         */self.close=function(){$state.go('fedramp.app.home.assessors',{},{reload:true});};helperService.scrollTo('scrollToContent');}})();(function(){'use strict';angular.module('fedramp').controller('AssessorsController',AssessorsController);AssessorsController.$inject=['$log','assessors'];/**
     * @constructor
     * @memberof Controllers
     */function AssessorsController($log,assessors){var self=this;self.sidebar=false;self.filteredData=[];self.assessors=assessors;self.reuseRangeOptions=reuseRangeOptions;self.reuseRangeFilter=reuseRangeFilter;self.onUpdate=onUpdate;self.heavyUsers=function(){var heavy=self.filteredData.filter(function(x){return x.reuses>=5;}).length;return Math.round(heavy/self.filteredData.length*100);};function onUpdate(items){self.filteredData=items;}function reuseRangeOptions(assessors){return[{value:{min:0,max:5},label:'0 - 5',selected:false},{value:{min:6,max:10},label:'5 - 10',selected:false},{value:{min:11,max:1000},label:'> 10',selected:false}];}function reuseRangeFilter(assessor,index,arr,selectedOptions){return selectedOptions.find(function(option){if(assessor.reuses>=option.value.min&&assessor.reuses<=option.value.max){return option;}});}}})();(function(){'use strict';angular.module('fedramp').controller('HomeController',HomeController);HomeController.$inject=['$log','fedrampData','CsvService'];/**
     * @constructor
     * @memberof Controllers
...
navigator.msSaveBlob(downloadBlob,self.filename());}};/**
     ...
     */function ProductComparisonController($log,$state,$stateParams,fedrampData,helperService){var self=this;/**
         ...
         * @member {object}
         * @memberof Controllers.ProductComparisonController
         */self.closeSecond=function(){$state.go('fedramp.app.'+self.first.type+'.information',{name:$stateParams.first},{reload:true});};helperService.scrollTo('scrollToContent');}})();(function(){'use strict';angular.module('fedramp').controller('ProductController',ProductController);ProductController.$inject=['$log','fedrampData'];/**
     * @constructor
     * @memberof Controllers
     ...
     */function ProductInformationController($log,$state,$stateParams,fedrampData,helperService){var self=this;/**
         ...
         * @member {object}
         * @memberof Controllers.ProductInformationController
         */self.close=function(){$state.go('fedramp.app.home.products',{},{reload:true});};helperService.scrollTo('scrollToContent');}})();(function(){"use strict";angular.module('fedramp').controller('ProductsController',ProductController);ProductController.$inject=['$log','products','$stateParams','$filter','$location'];function ProductController($log,products,$stateParams,$filter,$location){var self=this;self.onUpdate=onUpdate;self.products=products;self.productNameSearchFilterFunc=productNameSearchFilterFunc;function onUpdate(items){$log.debug('Incoming dataset');self.filteredData=items;}self.compliant=function(){var compliant=self.filteredData.filter(function(x){return x.designation==='Compliant'||x.designation==='In PMO Review';}).length;return Math.round(compliant/self.filteredData.length*100);};function productNameSearchFilterFunc(product,i,arr,searchTerm){// If no search term, we display result since this is the same as having nothing selected
if(!searchTerm){return product;}searchTerm=searchTerm.toLowerCase();var productName=product.name.toLowerCase();var providerName=product.provider.toLowerCase();if(productName.indexOf(searchTerm)!==-1||providerName.indexOf(searchTerm)!==-1){return product;}return null;}}})();(function(){'use strict';angular.module('fedramp').controller('ProviderComparisonController',ProviderComparisonController);ProviderComparisonController.$inject=['$log','$state','$stateParams','fedrampData','helperService'];/**
     ...
     */function ProviderComparisonController($log,$state,$stateParams,fedrampData,helperService){var self=this;/**
         ...
         * @member {object}
         * @memberof Controllers.ProviderComparisonController
         */self.closeSecond=function(){$state.go('fedramp.app.'+self.first.type+'.information',{name:$stateParams.first},{reload:true});};helperService.scrollTo('scrollToContent');}})();(function(){})();(function(){'use strict';angular.module('fedramp').controller('ProviderInformationController',ProviderInformationController);ProviderInformationController.$inject=['$log','$state','$stateParams','fedrampData','helperService'];/**
     * @constructor
     * @memberof Controllers
