...
var STATE_REGEX=/^fedramp.app.(?!home)/g;/**
         ...
         */$rootScope.$on('$stateChangeStart',function(event,toState,toParams,fromState,fromParams,options){params=$location.search();routeOptions=options;});$rootScope.$on('$stateChangeSuccess',function(event,toState,toParams,fromState,fromParams,options){// We check if the state we're coming from is NOT a child of the fedramp.app.home. Items that
...
var match=fromState.name.match(STATE_REGEX);if(match){// We check if the location: <boolean> was defined for a particular route change.
if(!routeOptions.location){$location.search(params);}else{$location.search({});}}});}})();(function(){'use strict';angular.module('fedramp').config(routeConfig);// Add items to inject for safe minification
...
.state('fedramp.app.home.assessors',{url:'/assessors',templateUrl:'src/templates/home/assessors.html',controller:'AssessorsController as controller',resolve:{assessors:['fedrampData',function(fedrampData){return fedrampData.assessors();}]}}).state('fedramp.app.assessor',{url:'/assessor',templateUrl:'src/templates/home/assessor.html'}).state('fedramp.app.assessor.information',{url:'/:name',templateUrl:'src/templates/home/assessor-information.html',controller:'AssessorInformationController as controller'}).state('fedramp.app.assessor.comparison',{url:'/:first/versus/:second',templateUrl:'src/templates/home/assessor-comparison.html',controller:'AssessorComparisonController as controller'});}})();(function(){'use strict';angular.module('fedramp.components').component('agency',{templateUrl:'src/templates/components/agency.html',controller:Agency,controllerAs:'controller',bindings:{model:'<',onClose:'<'}});Agency.$inject=['$log','$state','helperService'];/**
     ...
     */function Agency($log,$state,helperService){var self=this;/**
         ...
         */self.linkify=function(modelType,name){return'#/'+modelType+'/'+helperService.slugify(name)+helperService.queryString();};}})();(function(){'use strict';angular.module('fedramp.components').component('assessor',{templateUrl:'src/templates/components/assessor.html',controller:Assessor,controllerAs:'controller',bindings:{model:'<',onClose:'<'}});Assessor.$inject=['$log','$state','helperService'];/**
     * @constructor
     * @memberof Components
     */function Assessor($log,$state,helperService){var self=this;/**
         * Close the informational panel
         * @public
         ...
         * @returns
         *  The pach for a hyperlink
         */self.linkify=function(modelType,name){return'#/'+modelType+'/'+helperService.slugify(name)+helperService.queryString();};}})();(function(){'use strict';angular.module('fedramp.components').component('comparison',{transclude:{'first':'first','second':'second'},templateUrl:'src/templates/components/comparison.html',controller:Comparison,controllerAs:'controller',bindings:{onCloseBoth:'@'}});Comparison.$inject=[];/**
     * @constructor
     * @memberof Components
     */function Comparison(){var self=this;}})();(function(){'use strict';angular.module('fedramp.components').component('downloadCsv',{transclude:true,template:'<a href="{{controller.downloadUrl}}" download="{{controller.filename()}}" ng-click="controller.download()"><ng-transclude/></a>',controller:DownloadCsv,controllerAs:'controller',bindings:{// Contains array of information to be transformed into csv
...
if(navigator.msSaveBlob&&downloadBlob){navigator.msSaveBlob(downloadBlob,self.filename());}}/**
     ...
     */function GridFilterClear(){var self=this;self.clear=clear;/**
         ...
         * @public
         * @memberof Components
         */function clear(){self.gridController.clearFilters();}}})();(function(){"use strict";angular.module('fedramp.components').component('gridFilter',{controller:GridFilter,controllerAs:'controller',templateUrl:'src/templates/components/grid-filter.html',require:{// We require that this component live inside of <grid></grid> so it can
...
id:'@'}});GridFilter.$inject=['$location','$parse','$element','$httpParamSerializer','Searcher'];/**
     ...
     * @constructor
     * @memberof Components
     */function GridFilter($location,$parse,$element,$httpParamSerializer,Searcher){var self=this;var selectedCss='grid-filter-selected';var OBJECT_PARAM_REGEX=/\:\((.+?)\),{0,}/;// Default template used to render option values
self.gridFilterOptionsTemplatePath="src/templates/components/grid-filter-options.html";// Options available to filter based on property
...
self.$onInit=$onInit;self.applyFilter=applyFilter;self.selectOption=selectOption;self.clear=clear;self.loadOptions=loadOptions;self.restoreState=restoreState;function $onInit(){if(!self.id){throw'Please add an id attribute';}if(!self.property&&(!self.optionsFunc||!self.filterFunc)){throw'If property is not specified, optionsFunc and filterFunc must be passed in';}// Allow custom optionsFunc and filterFuncs to be passed for custom filtering
...
if(self.options.length===0){self.loadOptions(self.gridController.rawItems);}restoreState();if(self.expanded){$element.addClass('grid-filter-expanded');}}/**
...
if(self.selectedOptionValues.length===0){return obj;}return self.selectedOptionValues.find(function(option){var found=new Searcher().prop(self.property).equals(obj,option.value);if(found.length>0){return obj;}return null;});}/**
         ...
         * @public
         * @memberof Components.GridFilter
         */function toggleCss(){if(self.selectedOptionValues.length>0){$element.addClass(selectedCss);}else{$element.removeClass(selectedCss);}}}})();(function(){'use strict';angular.module('fedramp.components').component('gridSearch',{templateUrl:'src/templates/components/grid-search.html',controller:GridSearch,controllerAs:'controller',require:{gridController:'^grid'},bindings:{// Property expression. Tells search which property to search on. 
...
self.defaultSort=null;self.items=[];/**
         * Initializes the component.
         */function $onInit(){if(!self.onUpdate){throw'Specify an onUpdate function! Otherwise, you don\' get updates.';}self.items=self.rawItems;}/**
...
$location.search(self.state);self.onUpdate({items:combinedFilterResults,state:self.state});}/**
         ...
         */function stateUpdate(state){state=angular.extend(self.state,state);$location.search(state);self.onUpdate({items:self.items,state:state});}}})();(function(){'use strict';angular.module('fedramp.components').component('productList',{templateUrl:'src/templates/components/product-list.html',controller:ProductList,controllerAs:'controller',bindings:{products:'<'}});ProductList.$inject=['fedrampData','helperService'];/**
     * Renders a list of products with corresponding provider names. A link is generated for both provider and product.
     *
     * @constructor
     * @memberof Components
     */function ProductList(fedrampData,helperService){var self=this;self.productList=[];self.$onInit=$onInit;self.createList=createList;self.slugify=slugify;function $onInit(){createList();}function createList(){var providers=fedrampData.providers();self.products.reduce(function(arr,product){for(var x=0;x<providers.length;x++){var provider=providers[x];for(var y=0;y<provider.products.length;y++){var curProduct=provider.products[y];if(curProduct.name===product){arr.push({name:product,provider:provider.name});break;}}}return arr;},self.productList);}function slugify(slugme){return helperService.slugify(slugme);}}})();(function(){'use strict';angular.module('fedramp.components').component('productStatus',{templateUrl:'src/templates/components/product-status.html',controller:ProductStatus,controllerAs:'controller',bindings:{status:'<'}});ProductStatus.$inject=[];/**
     * Renders the appropriate fedramp status icon and text based on the inputted status text
     * @constructor
     * @memberof Components
     */function ProductStatus(){var self=this;}})();(function(){'use strict';angular.module('fedramp.components').component('product',{templateUrl:'src/templates/components/product.html',controller:Product,controllerAs:'controller',bindings:{model:'<',products:'<',onClose:'<'}});Product.$inject=['$log','$state','helperService'];/**
     * @constructor
     * @memberof Components
     */function Product($log,$state,helperService){var self=this;/**
         ...
         * @returns
         *  Status message
         */self.statusMessage=function(){var message='';if(self.isCompliant()){message='Compliant Since '+(self.model.authorizationDate||'');}else{if(self.model.expectedCompliance){message='Estimated Compliance Date '+self.model.expectedCompliance;}else{message='This provider has not given an Estimated Compliance Date';}}return message;};}})();(function(){'use strict';angular.module('fedramp.components').component('provider',{templateUrl:'src/templates/components/provider.html',controller:Provider,controllerAs:'controller',bindings:{model:'<',onClose:'<'}});Provider.$inject=['$log','$state','helperService'];/**
     * @constructor
     * @memberof Components
...
var self=this;/**
     ...
     * @constructor
     * @memberof Services
     */function DataService($log,StorageManager,StorageData,Data,DatasourceService){var self=this;var dataUrl='https://raw.githubusercontent.com/18F/fedramp-data/master/data/data.json';/**
     ...
     */function FedrampDataProvider(){var provider=this;provider.defaults={cache:true};/**
         ...
         *
         *
         */provider.$get=['$log','$cacheFactory',function($log,$cacheFactory){return new FedrampDataService();/**
             ...
             */function FedrampDataService(){var self=this;// Create a cache bucket
var cache=$cacheFactory('fedramp');/**
                 * Stores cache storage data.
                 */self.load=load;self.cacheWrap=cacheWrap;/**
...
if(provider.defaults.cache){self.products=cacheWrap('products')(storage.products);self.providers=cacheWrap('providers')(storage.providers);self.assessors=cacheWrap('assessors')(storage.assessors);self.agencies=cacheWrap('agencies')(storage.agencies);}//console.log(self.products());
}/*
                 ...
                 */function cacheWrap(key){return function(func){return function(){var data=cache.get(key);if(data){$log.debug(key,'cache hit!');return data;}$log.debug(key,'cache miss');var result=func.apply(null,arguments);cache.put(key,result);return result;};};}}}];}})();(function(){'use strict';angular.module('fedramp.services').service('helperService',HelperService);HelperService.$inject=['$log','$location'];/**
     * @constructor
     * @memberof Services
...
self.storageContainer='default';/**
     ...
     */function AgenciesController($log,agencies){var self=this;/**
         ...
         */self.reuseRangeFilter=function(agency,index,arr,selectedOptions){return selectedOptions.find(function(option){if(agency.products.length>=option.value.min&&agency.products.length<=option.value.max){return agency;}});};}})();(function(){'use strict';angular.module('fedramp').controller('AgencyComparisonController',AgencyComparisonController);AgencyComparisonController.$inject=['$log','$state','$stateParams','fedrampData','helperService'];/**
     * @constructor
     * @memberof Controllers
     ...
     */function AgencyInformationController($log,$state,$stateParams,fedrampData,helperService){var self=this;var agencies=fedrampData.agencies();/**
         ...
         * @returns
         *  The matched item or null
         */self.reuseRangeFilter=function(agency,index,arr,selectedOptions){return selectedOptions.find(function(option){if(agency.products.length>=option.value.min&&agency.products.length<=option.value.max){return option;}});};helperService.scrollTo('scrollToContent');}})();(function(){'use strict';angular.module('fedramp').controller('AssessorComparisonController',AssessorComparisonController);AssessorComparisonController.$inject=['$log','$state','$stateParams','fedrampData','helperService'];/**
     * @constructor
     * @memberof Controllers
...
if(navigator.msSaveBlob&&downloadBlob){navigator.msSaveBlob(downloadBlob,self.filename());}};/**
         ...
         */self.totalAuthorized=function(){return fedrampData.products().filter(function(x){return x.designation!=='In-Process';}).length;};/**
         ...
         */self.totalInProcess=function(){return fedrampData.products().filter(function(x){return x.designation==='In-Process';}).length;};/**
         ...
         */self.totalReady=function(){return 0;};}})();(function(){'use strict';angular.module('fedramp').controller('ProductComparisonController',ProductComparisonController);ProductComparisonController.$inject=['$log','$state','$stateParams','fedrampData','helperService'];/**
     * @constructor
     * @memberof Controllers
