...
var self=this;/**
     ...
     */function DatasourceService($http){var self=this;/**
         ...
         */function StorageData(options){StorageManager.call(this);var self=this;self.storageContainer='data';/**
             ...
             */self.products=function(){var names=[];var items=[];var data=self.all();for(var i=0;i<data.length;i++){var d=data[i];if(!d.pkg||names.includes(d.pkg)){continue;}names.push(d.pkg);var item=new Product();item.name=d.pkg;item.provider=d.name;item.serviceModels=d.serviceModel;item.deploymentModel=d.deploymentModel;item.designation=d.designation;item.impactLevel=d.impactLevel;items.push(item);}items.forEach(function(item){data.forEach(function(d){if(d.pkg===item.name){if(!item.agencies.includes(d.sponsoringAgency)){item.agencies.push(d.sponsoringAgency);}if(!item.agencies.includes(d.sponsoringSubagency)){item.agencies.push(d.sponsoringSubagency);}d.atoLetters.forEach(function(a){if(!item.agencies.includes(a.authorizingAgency)){item.agencies.push(d.authorizingAgency);}if(!item.agencies.includes(a.authorizingSubagency)){item.agencies.push(d.authorizingSubagency);}});}});});return items;};/**
...
summedReuses+=leveragedATOs.map(function(otherCSP){return otherCSP.atoLetters.length;}).reduce(function(prev,curr){return prev+curr;});}return directlyLeveraged+summedReuses;};/**
         ...
         */function sortData(){if(!self.sortBy||self.sortBy.length===0){return;}self.filteredData=self.filteredData.sort(function(a,b){if(a[self.sortBy]===b[self.sortBy]){return 0;}if(self.sortAscending){return a[self.sortBy]<b[self.sortBy]?-1:1;}return a[self.sortBy]>b[self.sortBy]?-1:1;});}}})();(function(){})();(function(){})();