...
var self=this;/**
     ...
     */function DatasourceService($http){var self=this;/**
         ...
         */function StorageData(options){StorageManager.call(this);var self=this;self.storageContainer='data';/**
             ...
             */self.providers=function(){var names=[];var items=[];var data=self.all();for(var i=0;i<data.length;i++){var d=data[i];if(!include(d.name,names)){continue;}names.push(d.name.trim());var item=new Provider();item.name=d.name.trim();items.push(item);}items.forEach(function(item){item.products=self.products().filter(function(x){return x.name===item.name;});item.products.forEach(function(prod){item.reuses+=prod.reuses;item.products.forEach(function(prod){prod.serviceModels.forEach(function(model){if(include(model,item.serviceModels)){item.serviceModels.push(model.trim());}});});item.products.forEach(function(prod){prod.deplomentModels.forEach(function(model){if(include(model,item.deploymentModels)){item.deploymentModels.push(model.trim());}});});item.products.forEach(function(prod){if(include(prod.deploymentModel,item.designations)){item.designations.push(prod.deploymentModel.trim());}});});});return items;};/**
             ...
             */self.products=function(){var names=[];var items=[];var data=self.all();for(var i=0;i<data.length;i++){var d=data[i];if(!include(d.pkg,names)){continue;}names.push(d.pkg.trim());var item=new Product();item.name=d.pkg.trim();item.provider=d.name.trim();item.pkgId=d.pkgId.trim();item.serviceModels=d.serviceModel;item.deploymentModel=d.deploymentModel.trim();item.designation=d.designation.trim();item.impactLevel=d.impactLevel.trim();items.push(item);}items.forEach(function(item){data.forEach(function(d){if(d.pkg===item.name){if(include(d.sponsoringAgency,item.agencies)){item.agencies.push(d.sponsoringAgency.trim());}if(include(d.sponsoringSubagency,item.agencies)){item.agencies.push(d.sponsoringSubagency.trim());}d.atoLetters.forEach(function(a){if(include(a.authorizingAgency,item.agencies)){item.agencies.push(a.authorizingAgency.trim());}if(include(a.authorizingSubagency,item.agencies)){item.agencies.push(a.authorizingSubagency.trim());}});}});});return items;};/**
...
for(var i=0;i<data.length;i++){var d=data[i];if(!include(d.independentAssessor,names)){continue;}names.push(d.independentAssessor.trim());var item=new Assessor();item.name=d.independentAssessor.trim();items.push(item);}// Nested
for(var _i3=0;_i3<data.length;_i3++){var _d2=data[_i3];for(var j=0;j<_d2.atoLetters.length;j++){var l=_d2.atoLetters[j];var _name='';if(!include(l.independentAssessor,names)){continue;}names.push(l.independentAssessor.trim());var _item4=new Assessor();_item4.name=l.independentAssessor.trim();items.push(_item4);}}items.forEach(function(item){data.forEach(function(d){if(safeTrim(d.independentAssessor)===item.name){if(include(d.pkg,item.products)){item.products.push(d.pkg.trim());}if(include(d.name,item.providers)){item.providers.push(d.name.trim());}if(include(d.sponsoringAgency,item.agencies)){item.agencies.push(d.sponsoringAgency.trim());}if(include(d.sponsoringSubagency,item.agencies)){item.agencies.push(d.sponsoringSubagency.trim());}}d.atoLetters.forEach(function(a){if(safeTrim(a.independentAssessor)===item.name){if(include(a.authorizingAgency,item.agencies)){item.agencies.push(a.authorizingAgency.trim());}if(include(a.authorizingSubagency,item.agencies)){item.agencies.push(a.authorizingSubagency.trim());}}});});});return items;};function safeTrim(s){if(s){return s.trim();}return'';}function include(s,a){var st=safeTrim(s);if(st&&a){return!a.includes(st);}return false;}return self.init(options);}StorageData.prototype=Object.create(StorageManager.prototype);StorageData.prototype.constructor=StorageData;return StorageData;}})();(function(){'use strict';angular.module('fedramp.services').factory('StorageManager',StorageManagerFactory);function StorageManagerFactory(){return StorageManager;}/**
