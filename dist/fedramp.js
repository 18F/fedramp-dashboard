...
if(filter.filtered.length===0){return;}combinedFilterResults=$filter('filter')(combinedFilterResults,filter.filterFunc);});self.items=combinedFilterResults;}function clearFilters(){filters.forEach(function(filter){filter.clear();});}/**
     ...
     * @constructor
     * @memberof Components
     */function Search($log,$state){var self=this;self.format='json';self.query='';self.search=function(e){e.preventDefault();$state.go('fedramp.app.search',{query:self.query},{reload:true});};}})();(function(){'use strict';angular.module('fedramp.components').component('tile',{templateUrl:'src/fedramp.components/tile.html',controller:Tile,controllerAs:'controller',bindings:{expand:'<',model:'<'}});Tile.$inject=['$log','$state','$stateParams','helperService'];/**
     * @constructor
     * @memberof Components
...
onInit();}})();(function(){'use strict';angular.module('fedramp').controller('ProviderComparisonController',ProviderComparisonController);ProviderComparisonController.$inject=['$log','$state','$stateParams','fedrampData','helperService'];/**
     ...
     * @constructor
     * @memberof Controllers
     */function ProvidersController($log,providers){var self=this;self.providers=providers;}})();(function(){'use strict';angular.module('fedramp').controller('SearchController',SearchController);SearchController.$inject=['$log','$http','$stateParams','fedrampData','helperService'];/**
     * @constructor
     * @memberof Controllers
     */function SearchController($log,$http,$stateParams,fedrampData,helperService){var self=this;self.results={};// FIXME: Currently not able to pull back results!!!
$http.get('https://search.usa.gov/search',{params:{utf8:'✓',affiliate:'fedramp',format:'json',query:$stateParams.query,commit:'Search'}}).then(function(response){if(response&&response.data){self.results=response.data;}});}})();(function(){'use strict';angular.module('fedramp').controller('SitemapController',SitemapController);SitemapController.$inject=['$log','fedrampData','helperService'];/**
     * @constructor
     * @memberof Controllers
