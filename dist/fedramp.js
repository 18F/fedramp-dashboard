...
values.split(',').forEach(function(val){selected.push({value:decodeURIComponent(val),selected:true});self.options.forEach(function(option){if(option.value===val){option.selected=true;}});});}self.selectedOptionValues=selected;if(self.selectedOptionValues){applyFilter();}}/**
...
return encodeURIComponent(option.value);}).join(',');self.gridController.state[self.id]=options;}else{delete self.gridController.state[self.id];}}/**
